...
Mira is a Rust MCP server that gives Claude Code persistent context, semantic code search, and context injection. It runs as a stdio-based MCP server spawned by Claude Code, backed by SQLite with sqlite-vec for vector operations.
...
| `context` | Context injection with budget management |
...
| `entities` | Heuristic entity extraction from subagent output |
