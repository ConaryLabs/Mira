...
fn setup_embedding_client() -> Arc<OpenAiEmbeddings> {
    ...
    Arc::new(OpenAiEmbeddings::new(api_key, "text-embedding-3-large".to_string()))
}

async fn setup_qdrant() -> Arc<QdrantMultiStore> {
    let qdrant_url = std::env::var("QDRANT_URL")
        .unwrap_or_else(|_| "http://localhost:6333".to_string());
    Arc::new(QdrantMultiStore::new(&qdrant_url, "test_code_embedding")
        .await
...
async fn test_parse_and_embed_code_elements() -> Result<()> {
    ...
    assert!(result.elements_count >= 2, "Should find at least 2 elements (function, struct)");
    ...
    assert_eq!(embedded_count, result.elements_count, "Should embed all parsed elements");
    
    // Step 4: Verify elements are in database
    let db_elements = sqlx::query_scalar::<_, i64>(
        "SELECT COUNT(*) FROM code_elements WHERE file_id = ?"
    )
    .bind(file_id)
    .fetch_one(&pool)
    .await?;
...
async fn test_semantic_search_code_elements() -> Result<()> {
    ...
    let results = multi_store.search(
        EmbeddingHead::Code,
    ...
    println!("[2] Search for 'authentication' returned {} results", results.len());
    assert!(!results.is_empty(), "Should find code elements related to authentication");
    
    // Verify results contain authentication function
    let has_auth_function = results.iter().any(|r| {
        r.content.contains("authenticate_user")
    });
    
    println!("[3] Results contain authenticate_user function: {}", has_auth_function);
...
async fn test_invalidation_on_file_change() -> Result<()> {
    ...
    assert_eq!(invalidated, original_count as u64, "Should invalidate all embeddings");
    ...
    code_intelligence
        .analyze_and_store_with_project(file_id, TEST_RUST_CODE_MODIFIED, "src/auth.rs", "rust", project_id)
        .await?;
    ...
    let has_async = results.iter().any(|r| {
        r.content.contains("async fn authenticate_user")
    });
...
async fn test_search_different_element_types() -> Result<()> {
    ...
    // Search for struct
    let struct_query = embedding_client.embed("configuration structure for authentication").await?;
    let struct_results = multi_store.search(
        EmbeddingHead::Code,
        &format!("code:{}", project_id),
        &struct_query,
        5
    ...
    let has_struct = struct_results.iter().any(|r| r.content.contains("struct AuthConfig"));
    println!("[2] Found AuthConfig struct: {}", has_struct);
    assert!(has_struct, "Should find struct");
    
    // Search for function
    let fn_query = embedding_client.embed("user authentication token verification").await?;
    let fn_results = multi_store.search(
        EmbeddingHead::Code,
        &format!("code:{}", project_id),
        &fn_query,
        5
    ...
    let has_function = fn_results.iter().any(|r| r.content.contains("fn authenticate_user"));
