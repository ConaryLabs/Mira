...
async fn test_protocol_coexistence() {
    ...
    tx.send(json!({"type": "status", "status": "thinking"})).await.unwrap();
    tx.send(json!({"type": "stream", "delta": "Legacy response"})).await.unwrap();
    ...
    tx.send(json!({"type": "stream", "delta": " continues"})).await.unwrap();
    tx.send(json!({
        "type": "operation.streaming",
        "operation_id": "op-concurrent",
        "delta": "Op response"
    ...
    let has_operations = received_messages.iter().any(|m| m["type"] == "operation.started");
...
async fn test_artifact_delivery_timing() {
    ...
    assert_eq!(received_messages[3]["artifacts"].as_array().unwrap().len(), 2);
...
async fn test_event_forwarding_no_double_wrapping() {
    ...
    for i in 0..3 {
        ...
        assert_ne!(msg["type"], "data", "Streaming messages should not be wrapped in data envelope");
    }
...
async fn test_full_message_router_integration() {
    ...
    tx.send(json!({"type": "status", "status": "thinking"})).await.unwrap();
    tx.send(json!({"type": "stream", "delta": "Hello!"})).await.unwrap();
    tx.send(json!({"type": "chat_complete", "content": "Hello!", "artifacts": []})).await.unwrap();
    
    println!("[2] Turn 2: Code generation (operations protocol)");
    tx.send(json!({"type": "operation.started", "operation_id": "op-1"})).await.unwrap();
    tx.send(json!({"type": "operation.streaming", "operation_id": "op-1", "delta": "Code"})).await.unwrap();
    tx.send(json!({
        "type": "operation.artifact_completed",
        "operation_id": "op-1",
        "artifact": {"id": "a1", "path": "test.rs", "content": "fn main() {}"}
    })).await.unwrap();
    tx.send(json!({"type": "operation.completed", "operation_id": "op-1"})).await.unwrap();
    
    println!("[3] Turn 3: Another simple chat");
    tx.send(json!({"type": "status", "status": "thinking"})).await.unwrap();
    tx.send(json!({"type": "stream", "delta": "Done!"})).await.unwrap();
    tx.send(json!({"type": "chat_complete", "content": "Done!", "artifacts": []})).await.unwrap();
    ...
    assert_eq!(received_messages.len(), 10, "Should receive all messages from 3 turns");
