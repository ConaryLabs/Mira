...
async fn test_profile_updates_from_llm() {
    ...
    let updated_profile = service.storage().get_or_create_profile(user_id).await
    ...
    assert_eq!(updated_profile.conversation_style, Some("casual".to_string()));
    assert_eq!(updated_profile.explanation_depth, Some("concise".to_string()));
...
async fn test_pattern_learning() {
    ...
    let patterns = service.storage().get_patterns(user_id).await
    ...
    let updated_patterns = service.storage().get_patterns(user_id).await
...
async fn test_pattern_filtering() {
    ...
    let coding_patterns = service.storage()
        .get_patterns_by_type(user_id, "coding_style").await
    ...
    let all_patterns = service.storage().get_patterns(user_id).await
...
async fn test_facts_crud() {
    ...
    let fact_id = facts_service.upsert_fact(
        user_id, "favorite_language", "rust", "preferences",
        Some("User mentioned in conversation"), 0.9
    ...
    let updated_id = facts_service.upsert_fact(
        user_id, "favorite_language", "rust and typescript",
        "preferences", None, 0.95
    ...
    facts_service.upsert_fact(user_id, "timezone", "PST", "personal", None, 1.0).await
        .expect("Failed to create timezone fact");
    facts_service.upsert_fact(user_id, "editor", "neovim", "preferences", None, 0.8).await
    ...
    facts_service.deprecate_fact(&fact_id).await
        .expect("Failed to deprecate fact");
    
    let deprecated = facts_service.get_fact(user_id, "favorite_language").await
...
async fn test_facts_from_llm() {
    ...
    assert_eq!(project_fact.source, Some("User mentioned in conversation".to_string()));
...
async fn test_context_loading() {
    ...
    facts_service.upsert_fact(user_id, "editor", "neovim", "preferences", None, 0.9).await
    ...
    assert_eq!(context.profile.conversation_style, Some("casual".to_string()));
...
async fn test_pattern_engine_updates() {
    ...
    service.process_llm_updates(user_id, Some(&initial_impact)).await
    ...
    service.process_llm_updates(user_id, Some(&reinforcement)).await
