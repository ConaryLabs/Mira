...
impl BranchManager {
    ...
    pub fn get_branches(&self, attachment: &GitRepoAttachment) -> Result<Vec<BranchInfo>> {
        ...
        for branch_result in git_branches {
            let (branch, _branch_type) =
                branch_result.into_api_error("Failed to process branch")?;

            if let Some(name) = branch.name().into_api_error("Failed to get branch name")? {
    ...
    pub fn get_commits(
        ...
        let mut revwalk = repo.revwalk().into_api_error("Failed to create revwalk")?;
