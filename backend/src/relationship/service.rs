...
use crate::relationship::{
    context_loader::ContextLoader, facts_service::FactsService, pattern_engine::PatternEngine,
...
impl RelationshipService {
    ...
    pub fn new(pool: Arc<SqlitePool>, facts_service: Arc<FactsService>) -> Self {
        ...
        let context_loader = Arc::new(ContextLoader::new(storage.clone(), pattern_engine.clone()));
    ...
    pub async fn process_llm_updates(
        ...
        if let Some(profile_changes) = impact_json
            .get("profile_changes")
            .and_then(|v| v.as_object())
        {
            self.process_profile_changes(user_id, profile_changes)
                .await?;
        }
    ...
    async fn process_profile_changes(
        ...
        if let Some(arch) = changes
            .get("architecture_preferences")
            .and_then(|v| v.as_str())
        {
    ...
    async fn process_new_patterns(
        ...
        for pattern_val in patterns {
            let pattern_type = pattern_val
                .get("pattern_type")
                .and_then(|v| v.as_str())
                .unwrap_or("general");
            let pattern_name = pattern_val
                .get("pattern_name")
                .and_then(|v| v.as_str())
                .unwrap_or("unnamed");
            let description = pattern_val
                .get("pattern_description")
                .and_then(|v| v.as_str())
                .unwrap_or("");
            let confidence = pattern_val
                .get("confidence")
                .and_then(|v| v.as_f64())
                .unwrap_or(0.7);
            let applies_when = pattern_val.get("applies_when").and_then(|v| v.as_str());
    ...
    async fn process_new_facts(&self, user_id: &str, facts: &[serde_json::Value]) -> Result<()> {
        for fact_val in facts {
            let fact_key = fact_val
                .get("fact_key")
                .and_then(|v| v.as_str())
                .unwrap_or("unknown");
            let fact_value = fact_val
                .get("fact_value")
                .and_then(|v| v.as_str())
                .unwrap_or("");
            let fact_category = fact_val
                .get("fact_category")
                .and_then(|v| v.as_str())
                .unwrap_or("general");
            let confidence = fact_val
                .get("confidence")
                .and_then(|v| v.as_f64())
                .unwrap_or(1.0);
            let context = fact_val.get("context").and_then(|v| v.as_str());
