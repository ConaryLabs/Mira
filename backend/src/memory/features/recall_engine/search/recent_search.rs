...
use crate::memory::{
    core::traits::MemoryStore, core::types::MemoryEntry, storage::sqlite::store::SqliteMemoryStore,
};
...
impl RecentSearch {
    ...
    pub async fn search(&self, session_id: &str, limit: usize) -> Result<Vec<ScoredMemory>> {
        debug!(
            "RecentSearch: Searching {} recent messages for session {}",
            limit, session_id
        );
