    ...
    async fn build_unified_result(
        ...
        if let Some(lang) = programming_lang.as_ref() {
            ...
            if !valid_langs.contains(&lang.to_lowercase().as_str()) {
                warn!(
                    "Language '{}' not in DB constraint - setting to NULL to avoid constraint violation",
                    lang
                );
                ...
                if is_code {
                    warn!(
                        "Detected code with unsupported language - treating as non-code for storage"
                    );
                    is_code = false;
                }
            }
        ...
        if is_code && programming_lang.is_none() {
            warn!(
                "LLM detected code but didn't specify valid language - treating as non-code to avoid DB constraint"
            );
            is_code = false;
        }
        ...
        if contains_error {
            info!(
                "LLM detected error - type: {:?}, file: {:?}, severity: {:?}",
        ...
        let routing = self
            .build_routing_decision(&chat_result, is_code, &programming_lang)
            .await?;
        ...
        if chat_result.salience < self.config.min_salience_threshold {
            return Ok(RoutingDecision {
                ...
                skip_reason: Some(format!(
                    ...
                    chat_result.salience, self.config.min_salience_threshold
                )),
            });
        }
