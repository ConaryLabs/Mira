// src/memory/features/prompts.rs
// System prompts for LLM-based memory features
// Note: In MCP mode, LLM calls go through Claude Code, but these
// prompts are used by internal analysis features

pub mod code_intelligence {
    /// System prompt for domain pattern analysis
    pub const DOMAIN_PATTERN_ANALYZER: &str =
        "You are a code analyzer that identifies domain patterns and suggests clustering.";

    /// System prompt for design pattern detection
    pub const DESIGN_PATTERN_DETECTOR: &str =
        "You are a code analyzer that detects design patterns in code.";

    /// System prompt for semantic analysis
    pub const SEMANTIC_ANALYZER: &str =
        "You are a code analyzer that identifies semantic relationships between code elements.";
...
pub mod analysis {
    ...
    pub const MESSAGE_ANALYZER: &str =
        "You are a message analyzer that extracts key information from conversations.";

    /// System prompt for batch analysis
    pub const BATCH_ANALYZER: &str =
        "You are a batch message analyzer that processes multiple messages efficiently.";
...
pub mod summarization {
    ...
    pub const ROLLING_SUMMARIZER: &str =
        "You are a summarization assistant that creates concise rolling summaries of conversations.";

    /// System prompt for snapshot summaries
    pub const SNAPSHOT_SUMMARIZER: &str =
        "You are a summarization assistant that creates comprehensive snapshot summaries.";
