...
impl MiraServer {
    ...
    async fn list_tables(&self) -> Result<CallToolResult, McpError> {
        let result = analytics::list_tables(self.db.as_ref()).await
    ...
    async fn remember(&self, Parameters(req): Parameters<RememberRequest>) -> Result<CallToolResult, McpError> {
        let result = memory::remember(self.db.as_ref(), self.semantic.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?;
        Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())]))
    }
    ...
    async fn recall(&self, Parameters(req): Parameters<RecallRequest>) -> Result<CallToolResult, McpError> {
        ...
        let result = memory::recall(self.db.as_ref(), self.semantic.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?;
        if result.is_empty() {
            Ok(CallToolResult::success(vec![Content::text(format!("No memories found matching '{}'", query))]))
        } else {
            Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())]))
        }
    }
    ...
    async fn forget(&self, Parameters(req): Parameters<ForgetRequest>) -> Result<CallToolResult, McpError> {
        let result = memory::forget(self.db.as_ref(), self.semantic.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?;
        Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())]))
    }
    ...
    async fn store_session(&self, Parameters(req): Parameters<StoreSessionRequest>) -> Result<CallToolResult, McpError> {
        let result = sessions::store_session(self.db.as_ref(), self.semantic.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?;
        Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())]))
    }
    ...
    async fn search_sessions(&self, Parameters(req): Parameters<SearchSessionsRequest>) -> Result<CallToolResult, McpError> {
        let query = req.query.clone();
        let result = sessions::search_sessions(self.db.as_ref(), self.semantic.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?;
        if result.is_empty() {
            Ok(CallToolResult::success(vec![Content::text(format!("No past sessions found matching '{}'", query))]))
        } else {
            Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())]))
        }
    }
    ...
    async fn store_decision(&self, Parameters(req): Parameters<StoreDecisionRequest>) -> Result<CallToolResult, McpError> {
        let result = sessions::store_decision(self.db.as_ref(), self.semantic.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?;
        Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())]))
    }
    ...
    async fn get_symbols(&self, Parameters(req): Parameters<GetSymbolsRequest>) -> Result<CallToolResult, McpError> {
        ...
        let result = code_intel::get_symbols(self.db.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?;
        if result.is_empty() {
            Ok(CallToolResult::success(vec![Content::text(format!("No symbols found in '{}'", file_path))]))
        } else {
            Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())]))
        }
    }
    ...
    async fn get_call_graph(&self, Parameters(req): Parameters<GetCallGraphRequest>) -> Result<CallToolResult, McpError> {
        let result = code_intel::get_call_graph(self.db.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?;
        Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())]))
    }
    ...
    async fn get_related_files(&self, Parameters(req): Parameters<GetRelatedFilesRequest>) -> Result<CallToolResult, McpError> {
        let result = code_intel::get_related_files(self.db.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?;
        Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())]))
    }
    ...
    async fn semantic_code_search(&self, Parameters(req): Parameters<SemanticCodeSearchRequest>) -> Result<CallToolResult, McpError> {
        let query = req.query.clone();
        let result = code_intel::semantic_code_search(self.db.as_ref(), self.semantic.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?;
        if result.is_empty() {
            Ok(CallToolResult::success(vec![Content::text(format!("No code found matching '{}'", query))]))
        } else {
            Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())]))
        }
    }
    ...
    async fn get_recent_commits(&self, Parameters(req): Parameters<GetRecentCommitsRequest>) -> Result<CallToolResult, McpError> {
        let result = git_intel::get_recent_commits(self.db.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?;
        if result.is_empty() {
            Ok(CallToolResult::success(vec![Content::text("No commits found")]))
        } else {
            Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())]))
        }
    }
    ...
    async fn search_commits(&self, Parameters(req): Parameters<SearchCommitsRequest>) -> Result<CallToolResult, McpError> {
        let query = req.query.clone();
        let result = git_intel::search_commits(self.db.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?;
        if result.is_empty() {
            Ok(CallToolResult::success(vec![Content::text(format!("No commits found matching '{}'", query))]))
        } else {
            Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())]))
        }
    }
    ...
    async fn find_cochange_patterns(&self, Parameters(req): Parameters<FindCochangeRequest>) -> Result<CallToolResult, McpError> {
        let file_path = req.file_path.clone();
        let result = git_intel::find_cochange_patterns(self.db.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?;
        if result.is_empty() {
            Ok(CallToolResult::success(vec![Content::text(format!("No co-change patterns found for '{}'", file_path))]))
        } else {
            Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())]))
        }
    }
    ...
    async fn find_similar_fixes(&self, Parameters(req): Parameters<FindSimilarFixesRequest>) -> Result<CallToolResult, McpError> {
        ...
        let result = git_intel::find_similar_fixes(self.db.as_ref(), self.semantic.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?;
        if result.is_empty() {
            Ok(CallToolResult::success(vec![Content::text(format!("No similar fixes found for: {}", error))]))
        } else {
            Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())]))
        }
    }
    ...
    async fn record_error_fix(&self, Parameters(req): Parameters<RecordErrorFixRequest>) -> Result<CallToolResult, McpError> {
        let result = git_intel::record_error_fix(self.db.as_ref(), self.semantic.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?;
        Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())]))
    }
    ...
    async fn get_build_errors(&self, Parameters(req): Parameters<GetBuildErrorsRequest>) -> Result<CallToolResult, McpError> {
        let result = build_intel::get_build_errors(self.db.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?;
        if result.is_empty() {
            Ok(CallToolResult::success(vec![Content::text("No build errors found")]))
        } else {
            Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())]))
        }
    }
    ...
    async fn record_build(&self, Parameters(req): Parameters<RecordBuildRequest>) -> Result<CallToolResult, McpError> {
        let result = build_intel::record_build(self.db.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?;
        Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())]))
    }
    ...
    async fn record_build_error(&self, Parameters(req): Parameters<RecordBuildErrorRequest>) -> Result<CallToolResult, McpError> {
        let result = build_intel::record_build_error(self.db.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?;
        Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())]))
    }
    ...
    async fn resolve_error(&self, Parameters(req): Parameters<ResolveErrorRequest>) -> Result<CallToolResult, McpError> {
        let result = build_intel::resolve_error(self.db.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?;
        Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())]))
    }
    ...
    async fn record_activity(&self, Parameters(req): Parameters<RecordActivityRequest>) -> Result<CallToolResult, McpError> {
        let result = workspace::record_activity(self.db.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?;
        Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())]))
    }
    ...
    async fn get_recent_activity(&self, Parameters(req): Parameters<GetRecentActivityRequest>) -> Result<CallToolResult, McpError> {
        let result = workspace::get_recent_activity(self.db.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?;
        if result.is_empty() {
            Ok(CallToolResult::success(vec![Content::text("No recent activity found")]))
        } else {
            Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())]))
        }
    }
    ...
    async fn set_context(&self, Parameters(req): Parameters<SetContextRequest>) -> Result<CallToolResult, McpError> {
        let result = workspace::set_context(self.db.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?;
        Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())]))
    }
    ...
    async fn get_context(&self, Parameters(req): Parameters<GetContextRequest>) -> Result<CallToolResult, McpError> {
        let result = workspace::get_context(self.db.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?;
        if result.is_empty() {
            Ok(CallToolResult::success(vec![Content::text("No active context")]))
        } else {
            Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())]))
        }
    }
    ...
    async fn get_guidelines(&self, Parameters(req): Parameters<GetGuidelinesRequest>) -> Result<CallToolResult, McpError> {
        let result = project::get_guidelines(self.db.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?;
        if result.is_empty() {
            Ok(CallToolResult::success(vec![Content::text("No guidelines found. Use 'add_guideline' to add project conventions.")]))
        } else {
            Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())]))
        }
    }
    ...
    async fn add_guideline(&self, Parameters(req): Parameters<AddGuidelineRequest>) -> Result<CallToolResult, McpError> {
        let result = project::add_guideline(self.db.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?;
        Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())]))
    }
    ...
    async fn create_task(&self, Parameters(req): Parameters<CreateTaskRequest>) -> Result<CallToolResult, McpError> {
        let result = tasks::create_task(self.db.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?;
        Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())]))
    }
    ...
    async fn list_tasks(&self, Parameters(req): Parameters<ListTasksRequest>) -> Result<CallToolResult, McpError> {
        let result = tasks::list_tasks(self.db.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?;
        if result.is_empty() {
            Ok(CallToolResult::success(vec![Content::text("No tasks found. Use 'create_task' to add a new task.")]))
        } else {
            Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())]))
        }
    }
    ...
    async fn get_task(&self, Parameters(req): Parameters<GetTaskRequest>) -> Result<CallToolResult, McpError> {
        ...
        match tasks::get_task(self.db.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))? {
            Some(result) => Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())])),
            None => Ok(CallToolResult::success(vec![Content::text(format!("Task {} not found", task_id))])),
        }
    }
    ...
    async fn update_task(&self, Parameters(req): Parameters<UpdateTaskRequest>) -> Result<CallToolResult, McpError> {
        let task_id = req.task_id.clone();
        match tasks::update_task(self.db.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))? {
            Some(result) => Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())])),
            None => Ok(CallToolResult::success(vec![Content::text(format!("Task {} not found", task_id))])),
        }
    ...
    async fn complete_task(&self, Parameters(req): Parameters<CompleteTaskRequest>) -> Result<CallToolResult, McpError> {
        let task_id = req.task_id.clone();
        match tasks::complete_task(self.db.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))? {
            Some(result) => Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())])),
            None => Ok(CallToolResult::success(vec![Content::text(format!("Task {} not found", task_id))])),
        }
    ...
    async fn delete_task(&self, Parameters(req): Parameters<DeleteTaskRequest>) -> Result<CallToolResult, McpError> {
        let task_id = req.task_id.clone();
        match tasks::delete_task(self.db.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))? {
            Some(title) => Ok(CallToolResult::success(vec![Content::text(
    ...
    // ========================================================================
    // Document Search Tools (with semantic search)
    // ========================================================================
    ...
    async fn list_documents(&self, Parameters(req): Parameters<ListDocumentsRequest>) -> Result<CallToolResult, McpError> {
        let result = documents::list_documents(self.db.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?;
        if result.is_empty() {
            Ok(CallToolResult::success(vec![Content::text("No documents found.")]))
        } else {
            Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())]))
        }
    }
    ...
    async fn search_documents(&self, Parameters(req): Parameters<SearchDocumentsRequest>) -> Result<CallToolResult, McpError> {
        let query = req.query.clone();
        let result = documents::search_documents(self.db.as_ref(), self.semantic.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?;
        if result.is_empty() {
            Ok(CallToolResult::success(vec![Content::text(format!("No document content found matching '{}'", query))]))
        } else {
            Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())]))
        }
    }
    ...
    async fn get_document(&self, Parameters(req): Parameters<GetDocumentRequest>) -> Result<CallToolResult, McpError> {
        ...
        match documents::get_document(self.db.as_ref(), req).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))? {
            Some(result) => Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&result).unwrap())])),
            None => Ok(CallToolResult::success(vec![Content::text(format!("Document '{}' not found", doc_id))])),
        }
    }
}
...
impl ServerHandler for MiraServer {
    fn get_info(&self) -> ServerInfo {
        ServerInfo {
            ...
            instructions: Some("Mira Power Suit - Memory and Intelligence Layer for Claude Code. Features: semantic memory (remember/recall across sessions), cross-session context (store_session/search_sessions/store_decision), persistent tasks, code intelligence (symbols, call graphs, semantic search), git intelligence (cochange, error fixes), and document RAG. All search tools use semantic similarity when Qdrant/OpenAI are configured.".to_string()),
        }
    }
}
