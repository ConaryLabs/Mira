...
// ============================================================================
// Refactor Recipe: Safe Restructuring
// ============================================================================

const REFACTOR_MEMBERS: &[RecipeMember] = &[
    // Phase 1: Analysis
    RecipeMember {
        name: "architect",
        agent_type: "general-purpose",
...
const REFACTOR_TASKS: &[RecipeTask] = &[
    RecipeTask {
    ...
    RecipeTask {
        ...
        description: "Verify all callers are accounted for, check for hidden behavior changes, validate step ordering, and assess test coverage. Rate: ready / needs revision / too risky.",
        assignee: "code-reviewer",
    },
    RecipeTask {
        subject: "Verify refactoring preserved behavior",
        description: "Run full test suite, clippy, fmt. Distinguish genuine regressions from tests needing path updates. Check for dead code left behind.",
        assignee: "test-runner",
    },
];

const REFACTOR_COORDINATION: &str = r#"## Refactor: Safe Restructuring
...
2. **Spawn architect and code-reviewer** in parallel using `Task` tool with `run_in_background=true`
   - Append the user's refactoring goal/context to each agent's prompt
3. **Create and assign analysis tasks** using `TaskCreate` + `TaskUpdate`
4. **Wait** for both agents to report via SendMessage
5. **Shut down** architect and code-reviewer (they're done)

### Phase 2: Synthesis

6. **Combine** architect's plan with code-reviewer's feedback:
...
7. **Present** the validated refactoring plan to the user and WAIT for approval

### Phase 3: Implementation

8. **Execute the refactoring steps** in order. For each step:
   - Make the changes
   - Run `cargo test --no-run` to verify compilation (NEVER use --release)
   - If it doesn't compile, fix before moving to the next step
9. For large refactors, you can spawn implementation agents with `mode="bypassPermissions"`:
...
10. **Spawn test-runner** with context about what was refactored
11. **Create and assign verification task**
12. **Wait** for test results
13. If tests fail:
    - Regressions: fix the refactoring step that caused it
    - Test path updates: update the test imports/paths
14. **Shut down** test-runner

### Phase 5: Finalize

15. **Run final checks**: `cargo clippy --all-targets --all-features -- -D warnings` AND `cargo fmt`
16. **Report** summary of all structural changes to the user
17. **Cleanup**: `TeamDelete`

### Important Notes

- Analysis agents (architect, code-reviewer) are READ-ONLY — do NOT give them `mode="bypassPermissions"`
- Implementation agents get `mode="bypassPermissions"`
...
const REFACTOR: Recipe = Recipe {
    name: "refactor",
    description: "Safe code restructuring: architect plans, code-reviewer validates, implementation with per-step compilation checks, test-runner verifies behavior preserved.",
    members: REFACTOR_MEMBERS,
    tasks: REFACTOR_TASKS,
    coordination: REFACTOR_COORDINATION,
};

/// All built-in recipes.
const ALL_RECIPES: &[&Recipe] = &[&EXPERT_REVIEW, &FULL_CYCLE, &QA_HARDENING, &REFACTOR];
