...
9. **Shut down** architect and code-reviewer (they're done)
...
12. **Execute the refactoring steps** yourself. For each step:
    - Make the changes
    - Run `cargo test --no-run` to verify compilation (NEVER use --release)
    - If it doesn't compile, fix before moving to the next step
13. For **large refactors** (5+ files), you can spawn implementation agents with `mode="bypassPermissions"`:
...
14. **Run `cargo test`** to verify all tests pass (NEVER use --release)
15. For **large refactors**, optionally spawn test-runner for parallel clippy + fmt + dead code checks
16. For **small/medium refactors**, just run `cargo test` and `cargo clippy` yourself — faster than spawning an agent
17. If tests fail:
...
18. **Report** summary of all structural changes to the user (files moved, line counts before/after)
