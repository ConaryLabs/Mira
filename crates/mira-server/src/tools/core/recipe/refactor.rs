...
pub(super) const MEMBERS: &[RecipeMember] = &[
    // Phase 1: Analysis (read-only, use sonnet)
    RecipeMember {
        name: "architect",
    ...
    // Phase 2: Validation (read-only, use sonnet)
    RecipeMember {
        name: "code-reviewer",
        agent_type: "general-purpose",
    ...
    // Phase 4: Verification (read-only commands, use sonnet)
    RecipeMember {
        name: "test-runner",
        agent_type: "general-purpose",
...
2. **Spawn architect** using `Task` tool with `team_name`, `name`, `subagent_type`, `model` (if present), and `run_in_background=true`
...
5. **Spawn code-reviewer** using `Task` tool with `team_name`, `name`, `subagent_type`, `model` (if present), and `run_in_background=true`
