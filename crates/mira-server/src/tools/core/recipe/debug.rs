...
pub(super) const MEMBERS: &[RecipeMember] = &[
    ...
    // Phase 2: Diagnose (sequential — receives symptom-analyzer's findings)
    RecipeMember {
    ...
    // Phase 3: Fix (parallel with regression-tester, after root-cause-analyst)
    RecipeMember {
        name: "fixer",
        agent_type: "general-purpose",
    ...
    // Phase 3: Regression test (parallel with fixer, after root-cause-analyst)
    RecipeMember {
...
3. **Spawn symptom-analyzer** using `Task` tool with `team_name`, `name`, `subagent_type="general-purpose"`, and `run_in_background=true`
...
6. **Spawn root-cause-analyst** using `Task` tool with `team_name`, `name`, `subagent_type="general-purpose"`, and `run_in_background=true`
   - Do NOT give bypassPermissions — this agent is read-only
...
11. **Spawn fixer and regression-tester in PARALLEL** using `Task` tool with `team_name`, `name`, `subagent_type="general-purpose"`, `run_in_background=true`, and `mode="bypassPermissions"` for both
