...
3. **Spawn coverage-analyst** using `Task` tool with `team_name`, `name`, `subagent_type="general-purpose"`, `model` (if present), and `run_in_background=true`
...
7. **Spawn both writers** in parallel using `Task` tool with `model` (if present — omit for writers to use parent model), `run_in_background=true`, and `mode="bypassPermissions"`
