...
pub async fn run() -> Result<()> {
    ...
    // Close the session and save timestamp
    {
        let pool_clone = pool.clone();
        ...
        let _ = pool_clone
            .interact(move |conn| {
                ...
                // Close the session (mark as completed, summary will be generated by background worker)
                if !session_id.is_empty() {
                    crate::db::close_session_sync(conn, &session_id, None).ok();
                    eprintln!("[mira] Closed session {}", &session_id[..session_id.len().min(8)]);
                }
                Ok::<_, anyhow::Error>(())
            })
            .await;
