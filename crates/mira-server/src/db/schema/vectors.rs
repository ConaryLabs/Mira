...
// Vector table migrations for code embeddings storage
...
/// Check if the embedding provider has changed and store the new provider.
///
/// Reads the stored provider from `server_state`. If it differs from
/// `current_provider` (or is absent on first run), stores the new provider
/// and returns `true` so the caller can invalidate code embeddings.
