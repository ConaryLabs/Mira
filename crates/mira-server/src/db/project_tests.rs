...
mod tests {
    ...
    async fn test_get_or_create_project_basic() {
        ...
        let (id, name) = db!(pool, |conn| get_or_create_project_sync(
            conn,
            "/test/path",
            Some("test-project")
        )
    ...
    async fn test_get_or_create_project_upsert() {
        let pool = setup_test_pool().await;
        let (id1, name1) = db!(pool, |conn| get_or_create_project_sync(
            conn,
            "/test/path",
            Some("project-one")
        )
        .map_err(Into::into));
        let (id2, name2) = db!(pool, |conn| get_or_create_project_sync(
            conn,
            "/test/path",
            Some("project-two")
        )
    ...
    async fn test_get_or_create_project_different_paths() {
        let pool = setup_test_pool().await;
        let (id1, _) = db!(pool, |conn| get_or_create_project_sync(
            conn, "/path1", None
        )
        .map_err(Into::into));
        let (id2, _) = db!(pool, |conn| get_or_create_project_sync(
            conn, "/path2", None
        )
        .map_err(Into::into));
        assert_ne!(id1, id2);
    ...
    async fn test_get_project_info_existing() {
        let pool = setup_test_pool().await;
        let (id, _) = db!(pool, |conn| get_or_create_project_sync(
            conn,
            "/test/path",
            Some("test-project")
        )
        .map_err(Into::into));
        let info = db!(pool, |conn| get_project_info_sync(conn, id)
            .map_err(Into::into))
        .unwrap();
    ...
    async fn test_get_project_info_nonexistent() {
        let pool = setup_test_pool().await;
        let info = db!(pool, |conn| get_project_info_sync(conn, 99999)
            .map_err(Into::into));
        assert!(info.is_none());
    }
    ...
    async fn test_list_projects_multiple() {
        let pool = setup_test_pool().await;
        db!(pool, |conn| get_or_create_project_sync(
            conn,
            "/path1",
            Some("project1")
        )
    ...
    async fn test_update_and_get_project_briefing() {
        let pool = setup_test_pool().await;
        let (project_id, _) = db!(pool, |conn| get_or_create_project_sync(
            conn,
            "/test/path",
            Some("test")
        )
        .map_err(Into::into));
        db!(pool, |conn| update_project_briefing_sync(
            conn,
            project_id,
            "abc123",
        ...
        let briefing = db!(pool, |conn| get_project_briefing_sync(conn, project_id)
            .map_err(Into::into))
        .unwrap();
    ...
    async fn test_mark_session_clears_briefing() {
        ...
        .map_err(Into::into));
        db!(pool, |conn| mark_session_for_briefing_sync(
            conn, project_id
        )
        .map_err(Into::into));
    ...
    async fn test_get_projects_for_briefing_check() {
        let pool = setup_test_pool().await;
        let (project1, _) = db!(pool, |conn| get_or_create_project_sync(
            conn,
            "/path1",
            Some("p1")
        )
        ...
        .map_err(Into::into));
        db!(pool, |conn| mark_session_for_briefing_sync(conn, project1)
            .map_err(Into::into));

        let projects = db!(pool, |conn| get_projects_for_briefing_check_sync(conn)
            .map_err(Into::into));
    ...
    async fn test_set_and_get_server_state() {
        let pool = setup_test_pool().await;
        db!(pool, |conn| set_server_state_sync(
            conn,
            "test_key",
            "test_value"
        )
        .map_err(Into::into));
        let value = db!(pool, |conn| get_server_state_sync(conn, "test_key")
            .map_err(Into::into));
    ...
    async fn test_get_server_state_nonexistent() {
        let pool = setup_test_pool().await;
        let value = db!(pool, |conn| get_server_state_sync(conn, "nonexistent")
            .map_err(Into::into));
    ...
    async fn test_set_server_state_upsert() {
        let pool = setup_test_pool().await;
        db!(pool, |conn| set_server_state_sync(conn, "key", "value1")
            .map_err(Into::into));
        db!(pool, |conn| set_server_state_sync(conn, "key", "value2")
            .map_err(Into::into));
        let value = db!(pool, |conn| get_server_state_sync(conn, "key")
            .map_err(Into::into));
    ...
    async fn test_delete_server_state() {
        let pool = setup_test_pool().await;
        db!(pool, |conn| set_server_state_sync(conn, "key", "value")
            .map_err(Into::into));
        let deleted = db!(pool, |conn| delete_server_state_sync(conn, "key")
            .map_err(Into::into));
        assert!(deleted);
        let value = db!(pool, |conn| get_server_state_sync(conn, "key")
            .map_err(Into::into));
    ...
    async fn test_delete_server_state_nonexistent() {
        let pool = setup_test_pool().await;
        let deleted = db!(pool, |conn| delete_server_state_sync(conn, "nonexistent")
            .map_err(Into::into));
        assert!(!deleted);
    }
    ...
    async fn test_save_and_get_active_project() {
        let pool = setup_test_pool().await;
        db!(pool, |conn| save_active_project_sync(conn, "/my/project")
            .map_err(Into::into));
        let project = db!(pool, |conn| get_last_active_project_sync(conn)
            .map_err(Into::into));
    ...
    async fn test_clear_active_project() {
        ...
        db!(pool, |conn| save_active_project_sync(conn, "/my/project")
            .map_err(Into::into));
        db!(pool, |conn| clear_active_project_sync(conn)
            .map_err(Into::into));
        let project = db!(pool, |conn| get_last_active_project_sync(conn)
            .map_err(Into::into));
    ...
    async fn test_update_active_project() {
        let pool = setup_test_pool().await;
        db!(pool, |conn| save_active_project_sync(conn, "/project1")
            .map_err(Into::into));
        db!(pool, |conn| save_active_project_sync(conn, "/project2")
            .map_err(Into::into));
        let project = db!(pool, |conn| get_last_active_project_sync(conn)
            .map_err(Into::into));
    ...
    async fn test_full_project_lifecycle() {
        ...
        let info = db!(pool, |conn| get_project_info_sync(conn, project_id)
            .map_err(Into::into))
        .unwrap();
    ...
    async fn test_empty_project_path() {
        let pool = setup_test_pool().await;
        let (id, _name) = db!(pool, |conn| get_or_create_project_sync(conn, "", None)
            .map_err(Into::into));
        assert!(id > 0);
    }
    ...
    async fn test_very_long_project_name() {
        ...
        let (id, stored_name) = db!(pool, |conn| get_or_create_project_sync(
            conn,
            "/test",
            Some(&long_name_clone)
        )
        .map_err(Into::into));
        assert!(id > 0);
    ...
    async fn test_empty_server_state_key() {
        let pool = setup_test_pool().await;
        db!(pool, |conn| set_server_state_sync(conn, "", "value")
            .map_err(Into::into));
        let value = db!(pool, |conn| get_server_state_sync(conn, "")
            .map_err(Into::into));
    ...
    async fn test_empty_server_state_value() {
        let pool = setup_test_pool().await;
        db!(pool, |conn| set_server_state_sync(conn, "key", "")
            .map_err(Into::into));
        let value = db!(pool, |conn| get_server_state_sync(conn, "key")
            .map_err(Into::into));
