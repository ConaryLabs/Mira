...
fn scan_unused_functions(db: &Database, project_id: i64) -> Result<usize, String> {
    ...
    let unused: Vec<(String, String, i64)> = {
        ...
        // Exclude: test functions, common entry points, trait implementations
        let mut stmt = conn
            .prepare(
                "SELECT s.name, s.file_path, s.start_line
                 ...
                 WHERE s.project_id = ?
                   ...
                   AND s.name NOT IN (
                       SELECT DISTINCT callee_name FROM call_graph WHERE project_id = ?
                   )
                   AND s.name NOT LIKE 'test_%'
                   AND s.name NOT LIKE '%_test'
                   AND s.name NOT LIKE '%_tests'
                   AND s.name NOT IN ('main', 'run', 'new', 'default', 'from', 'into', 'drop', 'clone', 'fmt', 'eq', 'hash')
        ...
        stmt.query_map(params![project_id, project_id], |row| {
    ...
    for (name, file_path, line) in unused {
        ...
        db.store_memory(
            ...
            "health",
            Some("unused"),
            0.6, // Lower confidence - might be called via dynamic dispatch or macros
        )
        .map_err(|e| e.to_string())?;
