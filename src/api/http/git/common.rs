...
pub async fn get_validated_attachment(
    ...
    if attachment.project_id != project_id {
        return Err(ApiError::forbidden(
            "Attachment belongs to different project",
        ));
    }
...
pub fn should_index_file(path: &Path) -> bool {
    // THE FIX: Check path components to correctly identify ignored directories.
    // Using `to_string_lossy` and `contains` is brittle. Iterating through components is more robust.
