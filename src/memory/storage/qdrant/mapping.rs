...
pub fn memory_entry_to_payload(entry: &MemoryEntry) -> Value {
    json!({
        ...
        "contains_error": entry.contains_error,
        "error_type": entry.error_type,
        "error_severity": entry.error_severity,
...
pub fn payload_to_memory_entry(payload: &Value, vector: &[f32], id: Option<i64>) -> MemoryEntry {
    ...
    MemoryEntry {
        ...
        recall_count: payload
            .get("recall_count")
            .and_then(|v| v.as_i64()),
        contains_error: payload.get("contains_error").and_then(|v| v.as_bool()),
        error_type: payload
            .get("error_type")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string()),
        error_severity: payload
            .get("error_severity")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string()),
        error_file: payload
