...
impl QdrantMemoryStore {
        ...
        match client.get(&collection_url).send().await {
            ...
            _ => {
                ...
                if !resp.status().is_success() {
                    return Err(anyhow!("Failed to create Qdrant collection: {}", 
    ...
    pub async fn search_similar_memories(
        ...
        info!("🔍 Searching similar vectors in Qdrant for session: {}", session_id);

        let search_url = format!("{}/collections/{}/points/search", 
        ...
        if !response.status().is_success() {
            return Err(anyhow!("Qdrant search failed: {}", 
    ...
    fn payload_to_memory_entry(&self, payload: &serde_json::Map<String, Value>) -> Result<MemoryEntry> {
...
impl MemoryStore for QdrantMemoryStore {
    async fn save(&self, entry: &MemoryEntry) -> Result<()> {
        ...
        let upsert_url = format!("{}/collections/{}/points", 
            self.base_url, self.collection_name);
        ...
        if !response.status().is_success() {
            return Err(anyhow!("Qdrant save failed: {}", 
                response.text().await.unwrap_or_default()));
        }
        
        info!("✅ Saved memory to Qdrant (salience: {:?})", entry.salience);
        Ok(())
    }
