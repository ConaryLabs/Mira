...
async fn test_message_analysis_flow() {
    ...
    // Execute analysis
    let result = pipeline.analyze_message(test_content, role, None)
        .await
        .expect("Analysis should succeed");
    
    // Verify analysis structure
    assert!(result.should_embed, "Code-related message should be embedded");
    ...
    assert!(result.analysis.salience <= 1.0, "Salience should be normalized");
    
    // Verify code detection
    assert!(result.analysis.is_code, "Should detect code content");
    assert!(result.analysis.programming_lang.is_some(), "Should detect language");
    
    // Check that embedding routing was decided
    assert!(result.analysis.routing.should_embed, "Should route for embedding");
    assert!(!result.analysis.routing.embedding_heads.is_empty(), "Should have routing heads");
...
async fn test_low_value_message_skips_embedding() {
    ...
    for test_content in trivial_messages {
        let result = pipeline.analyze_message(test_content, "user", None)
            .await
            .expect("Analysis should succeed");
        
        // Trivial messages should not be embedded
        assert!(!result.should_embed, 
            "Trivial message '{}' shouldn't be embedded", test_content);
        assert!(result.analysis.salience < 0.5, 
            "Trivial message '{}' should have low salience", test_content);
    }
...
async fn test_high_value_messages_are_embedded() {
    ...
    for test_content in high_value_messages {
        ...
        assert!(result.should_embed, 
            "High-value message should be embedded: {}", test_content);
        assert!(result.analysis.salience > 0.5, 
            "High-value message should have high salience: {}", test_content);
    }
...
async fn test_error_detection_in_messages() {
    ...
    let error_messages = vec![
        ("Error: panic at 'index out of bounds' in src/main.rs:42", Some("src/main.rs")),
        ("TypeError: Cannot read property 'id' of undefined at app.js:156", Some("app.js")),
        ("Segmentation fault (core dumped) in parser.c line 89", Some("parser.c")),
        ("NullPointerException in AuthService.java:234", Some("AuthService.java")),
    ];
    
    for (test_content, expected_file) in error_messages {
        let result = pipeline.analyze_message(test_content, "assistant", None)
            .await
            .expect("Analysis should succeed");
        
        // Should detect error
        assert!(result.analysis.contains_error, 
            "Should detect error in: {}", test_content);
        
        // Should classify error type
        assert!(result.analysis.error_type.is_some(), 
            "Should classify error type in: {}", test_content);
        
        // Should extract file if present
        if let Some(expected) = expected_file {
            assert!(result.analysis.error_file.is_some(), 
                "Should extract error file from: {}", test_content);
            let extracted_file = result.analysis.error_file.as_ref().unwrap();
            assert!(extracted_file.contains(expected), 
                "Expected file '{}', got '{}'", expected, extracted_file);
        }
        
        // Errors are high-value
        assert!(result.should_embed, 
            "Error messages should be embedded: {}", test_content);
    }
...
async fn test_topic_extraction() {
    ...
    let result = pipeline.analyze_message(message_with_topics, "user", None)
        .await
        .expect("Analysis should succeed");
    
    // Should extract multiple topics
    assert!(!result.analysis.topics.is_empty(), "Should extract topics");
    assert!(result.analysis.topics.len() >= 2, "Should extract multiple topics");
    ...
    assert!(has_redis || has_architecture, 
        "Should extract relevant topics. Got: {:?}", result.analysis.topics);
...
async fn test_programming_language_detection() {
    ...
    let language_tests = vec![
        ("Here's a Python function using async/await", vec!["python", "py"]),
        ("This Rust function uses Result<T, E> for error handling", vec!["rust", "rs"]),
        ("JavaScript arrow function: const add = (a, b) => a + b", vec!["javascript", "js"]),
        ("SQL query: SELECT * FROM users WHERE active = true", vec!["sql"]),
    ];
    
    for (message, expected_langs) in language_tests {
        let result = pipeline.analyze_message(message, "user", None)
            .await
            .expect("Analysis should succeed");
        
        if result.analysis.is_code {
            let detected = result.analysis.programming_lang.as_ref()
                .expect("Should detect language for code content");
            
            let detected_lower = detected.to_lowercase();
            let matched = expected_langs.iter()
...
async fn test_mood_and_intent_analysis() {
    ...
    assert!(result.analysis.intensity.is_some(), "Should have intensity score");
...
async fn test_salience_scoring_consistency() {
    ...
    let high_salience = "Critical security vulnerability found in authentication system. Immediate patch required.";
    let high_result = pipeline.analyze_message(high_salience, "user", None)
        .await
        .expect("Should analyze high salience message");
    
    // Low salience content
    let low_salience = "ok thanks";
    let low_result = pipeline.analyze_message(low_salience, "user", None)
        .await
        .expect("Should analyze low salience message");
    
    // Verify relative salience
    assert!(high_result.analysis.salience > low_result.analysis.salience,
        "High-value content should have higher salience. High: {}, Low: {}",
        high_result.analysis.salience, low_result.analysis.salience);
    
    assert!(high_result.analysis.salience > 0.7, 
        "Critical content should have high salience score");
    assert!(low_result.analysis.salience < 0.3, 
...
async fn test_analysis_version_tracking() {
    ...
    let result = pipeline.analyze_message("test message", "user", None)
        .await
        .expect("Analysis should succeed");
    
    // Should have analysis version for tracking
    assert!(!result.analysis.analysis_version.is_empty(), 
...
async fn test_context_enhanced_analysis() {
    ...
    let context = Some("We were discussing implementing a retry mechanism with exponential backoff");
    
    let result = pipeline.analyze_message(content, "user", context)
        .await
        .expect("Analysis should succeed");
    
    // With context, salience should be higher than without
    let result_no_context = pipeline.analyze_message(content, "user", None)
        .await
        .expect("Analysis should succeed");
    
    // Context should improve understanding
    assert!(result.analysis.salience >= result_no_context.analysis.salience,
