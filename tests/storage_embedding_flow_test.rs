...
async fn test_sqlite_storage_retrieval() {
    ...
    // Store in SQLite
    let saved = sqlite.save(&entry)
        .await
        .expect("Should store in SQLite");
    ...
    let recent = sqlite.load_recent("test-session", 10)
        .await
...
async fn test_sqlite_storage_with_tags() {
    ...
    let saved = sqlite.save(&entry)
        .await
        .expect("Should store with tags");
    
    // Retrieve and verify tags
    let recent = sqlite.load_recent("test-session", 1)
        .await
...
async fn test_parent_child_relationships() {
    ...
    assert_eq!(child_saved.parent_id, Some(parent_id), 
...
async fn test_qdrant_connection() {
    ...
    assert!(!enabled_heads.is_empty(), "Should have enabled embedding heads");
...
async fn test_embedding_storage_in_qdrant() {
    ...
    let embedding = embedding_client.embed(test_content)
        .await
        .expect("Should generate embedding");
    
    assert_eq!(embedding.len(), 3072, "Should have correct embedding dimension");
    ...
    let point_id = multi_store.save(EmbeddingHead::Semantic, &entry)
        .await
...
async fn test_qdrant_semantic_search() {
    ...
    let query_embedding = embedding_client.embed(query).await.expect("Should embed query");
...
async fn test_multi_head_storage() {
    ...
    let embedding = embedding_client.embed(code_content).await.expect("Should embed");
    ...
    let results = multi_store.search(
        ...
        &embedding,
        1
    ).await.expect("Should search Code head");
...
async fn test_full_storage_flow() {
    ...
    let mut entry = create_test_entry("test-session", "user", 
    ...
    let embedding = embedding_client.embed(&entry.content)
        .await
        .expect("Should generate embedding");
    ...
    let point_id = multi_store.save(EmbeddingHead::Semantic, &qdrant_entry)
        .await
    ...
    let from_sqlite = sqlite.load_recent("test-session", 1)
        .await
    ...
    let search_results = multi_store.search(
        ...
        &embedding,
        1
    ).await.expect("Should search Qdrant");
...
async fn test_deletion_across_systems() {
    ...
    let embedding = embedding_client.embed(&entry.content).await.expect("Should embed");
    ...
    // Delete from SQLite
    sqlite.delete(message_id).await.expect("Should delete from SQLite");
    
    // Verify deletion from SQLite
    let recent = sqlite.load_recent("test-session", 10).await.expect("Should load");
    assert!(recent.iter().all(|e| e.id != Some(message_id)), 
        "Should not find deleted message in SQLite");
    
    // Delete from Qdrant
    multi_store.delete_from_all(message_id)
        .await
